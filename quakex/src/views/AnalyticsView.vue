...
/**
 * Export complete report with all data (JSONs)
 */
async function exportCompleteReport() {
    try {
        console.log('[AnalyticsView] Starting complete data export...')
        ...
        const rawDataFolder = zip.folder('raw-data')

        // Export JSON files from storage
        try {
            const response = await fetch('http://localhost:3001/api/files')
            const result = await response.json()

            if (result.success && result.files && result.files.length > 0) {
                console.log(`[AnalyticsView] Found ${result.files.length} JSON files to export`)
                ...
                for (const file of result.files) {
                    try {
                        ...
                        if (fileData.success && fileData.data) {
                            ...
                            rawDataFolder.file(file.name, jsonString)
                            console.log(`[AnalyticsView] Added JSON: ${file.name}`)
                        }
                    } catch (err) {
                        console.warn(`[AnalyticsView] Could not fetch JSON file: ${file.name}`, err)
                    }
                }
            }
        } catch (err) {
            console.warn('[AnalyticsView] Could not fetch JSON files from storage', err)
        }

        // Add README with report information
        const readme = 
        `QuakeX Analytics Report
        ========================

        Generated: ${new Date().toLocaleString()}
        Period: ${periodText.value}
        Total Events: ${totalEvents.value}

        Contents:
        ---------
        /raw-data/        - Raw enriched earthquake data (JSON format)

        Analysis Period:
        ----------------
        ${months.value.map((m) => formatMonth(m)).join(', ')}
        ...
        - Apache Drill Analytics Engine : https://drill.apache.org/docs/
        -------------
        Generated by QuakeX - Earthquake Data Explorer
        https://github.com/valeriehernandez-7/QuakeX-EarthquakExplorer
        `
        ...
        link.href = url
        link.download = `QuakeX-Data-From${months.value[2]}_To${months.value[1]}.zip`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        URL.revokeObjectURL(url)

        console.log('[AnalyticsView] Complete data exported successfully!')
    } catch (error) {
        console.error('[AnalyticsView] Error exporting complete data:', error)
    }
}
